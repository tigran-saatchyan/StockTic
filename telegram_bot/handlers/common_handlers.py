"""This module defines common handlers for the Telegram bot.

Functions:
    cmd_start: Handler for the /start command.
    process_register_yes: Handler for the registration confirmation.
    process_register_no: Handler for the registration rejection.
    cmd_clear: Handler for the /clear command.
    get_topic_id: Handler for the /get_topic_id command.
"""
...
async def process_register_no(
    ...
    sent_message = await callback_query.message.answer(
        "You can start the registration process later using "
        "the command /register or through the menu."
    )
    await callback_query.answer()
...
async def cmd_clear(message: types.Message, state: FSMContext) -> None:
    ...
    error_count = 0
    max_errors = (
        1  # Limit the number of consecutive errors to prevent infinite looping
    )

    for msg_id in range(
        message_id, message_id - 1000, -1
        ...
        try:
            ...
            error_count = (
                0  # Reset error count if a message is successfully deleted
            )
...
async def get_topic_id(message: types.Message) -> None:
    ...
    This handler retrieves the chat ID and the topic ID (message thread
    ID) from the incoming message
    and sends them back to the user. It also logs this information.

    Args:
        message (types.Message): The incoming message object containing
        the chat and thread information.
    """
