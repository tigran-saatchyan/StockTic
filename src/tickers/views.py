...
def ticker_list(request):
    ...
    return render(
        request, 'tickers/ticker_list.html',
...
def export_tickers(request):
    ...
    for ticker in Ticker.objects.all():
        writer.writerow(
            [ticker.symbol, ticker.name, ticker.country, ticker.ipo_year,
...
def get_stock_info(request, symbol):
    if request.method == "GET":
        ...
        return render(
            request, "tickers/company_profile.html",
