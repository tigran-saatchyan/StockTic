...
def watchlist_list(request):
    ...
    return render(request, "watchlist/watchlist_list.html", {"watchlists": watchlists})
...
def watchlist_detail(request, pk):
    ...
    for ticker in tickers:
        ...
        if history is not None:
            ...
            formatted_market_cap = (
                format_market_cap(market_cap) if market_cap else "N/A"
            )

            (
                day_range_position,
                year_range_position,
            ) = calculate_the_day_and_year_range_position(
    ...
    return render(
        request,
        "watchlist/watchlist_detail.html",
...
def calculate_the_day_and_year_range_position(
    ...
    if day_low is not None and day_high is not None and current_price is not None:
        day_range_position = (current_price - day_low) / (day_high - day_low) * 100
    else:
        day_range_position = (
            None  # Set to None or a default value if calculation is not possible
        )
    # Calculate the year range position
    if year_low is not None and year_high is not None and current_price is not None:
        year_range_position = (current_price - year_low) / (year_high - year_low) * 100
    else:
...
def add_ticker(request, pk):
    ...
    if request.method == "POST":
        ...
        form = AddTickerForm()
    return render(
        request, "watchlist/add_ticker.html", {"form": form, "watchlist": watchlist}
    )
...
class WatchlistTickerDeleteView(DeleteView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy("watchlist:watchlist_detail", kwargs={"pk": watchlist_pk})

    def get_object(self, queryset=None):
        watchlist_pk = self.kwargs["watchlist_pk"]
        ticker_symbol = self.kwargs["ticker_symbol"]
        return get_object_or_404(
            WatchlistTicker, watchlist_id=watchlist_pk, ticker__symbol=ticker_symbol
        )
